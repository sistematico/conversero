...
class Chat extends Model
{
    ...
    {
        $query = $this->db->prepare("SELECT chat.id AS id, chat.usuario AS usuario, chat.mensagem AS mensagem, chat.timestamp AS timestamp, chat.idusuario AS idusuario, usuarios.cor AS cor FROM chat,usuarios ORDER BY timestamp DESC LIMIT 20");
        $query->execute();
        $results = [];
