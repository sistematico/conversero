...
class Chat extends Model
{
    public function list()
    {
        ...
        $query = $this->db->prepare("SELECT chat.id AS id, chat.usuario AS usuario, chat.mensagem AS mensagem, chat.timestamp AS timestamp, usuarios.cor AS cor FROM chat LEFT JOIN usuarios ON chat.id = usuarios.id ORDER BY chat.timestamp DESC LIMIT 20");
        $query->execute();
        $results = [];
